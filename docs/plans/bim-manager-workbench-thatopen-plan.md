...
Статус: выполнено — `rules.ts` разделён на rule registry и grouped modules; добавлен список правил с включением/выключением и приоритизацией; добавлено управление и сохранение настроек проверок в BIM-профиле через localStorage; проверки покрыты unit-тестами.
...
**Статус:** выполнено — введён `DrawingDocument`/`SheetDocument`, аннотации и листы сведены к общей data model, экспорт и сохранение опираются на единый document layer; добавлен режим *Оформление* со split-view для модели и чертежа, а также синхронное выделение между моделью и чертежом.
